...
export const AIAgentLive: React.FC = () => {
  ...
  return (
    <section ref={ref} id="agent" className={`py-24 border-t transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'} ${isLight ? 'bg-white border-zinc-200' : 'bg-black border-zinc-900'}`}>
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          <div className="lg:col-span-5 space-y-5">
            ...
            <h2 className={`text-3xl sm:text-4xl font-extrabold tracking-tight leading-tight ${isLight ? 'text-zinc-900' : 'text-white'}`}>
              {t.agent.title}
            </h2>
          ...
          <div className="lg:col-span-7">
            <TerminalWindow
              ...
              statusBadge={
                <span className={`font-mono text-xs flex items-center gap-1.5 ${isLight ? 'text-cyan-600' : 'text-cyan-400'}`}>
            ...
            >
              <div className="space-y-2.5 font-mono text-xs max-h-[340px] overflow-y-auto pr-1">
                {logs.map((log) => (
                  ...
                  >
                    <span className={`font-mono text-[11px] ${isLight ? 'text-zinc-500' : 'text-zinc-500'}`}>{log.time}</span>
                    <span
                      className={`text-[11px] ${
                        log.status === 'alert'
